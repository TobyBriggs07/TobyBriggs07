...
function init(){
  ...
  function sizeAllThumbnails() {
    ...
    // **Force last row to stretch** if it exists
    if(_currentRowContents.length > 0){
        _currentRowOffsetTop += resizeRow(_currentRowContents, _availableWidth, _currentRowWidth);
        for(var j = 0; j < _currentRowContents.length; j++){
            _currentRowContents[j].$thumbnail.data("currentRowOffsetTop", _currentRowOffsetTop);
        }
...
function resizeRow(rowArray, availableWidth, currentWidth){
    ...
    for(var i = 0; i < rowArray.length; i++){
        totalAspectRatio += rowArray[i].aspectRatio;
    }

    var spacing = 8; // spacing in px between images
    var rowSpacingTotal = spacing * (rowArray.length - 1);
    var newHeight = Math.floor((availableWidth - rowSpacingTotal) / totalAspectRatio);

    var _newCurrentWidth = 0;
    for(var i = 0; i < rowArray.length; i++){
        var _thumbWidth = Math.floor(newHeight * rowArray[i].aspectRatio);
        var _thumbHeight = newHeight;

        // Add spacing margin
        rowArray[i].$thumbnail.css({
            "width": _thumbWidth + "px",
            "height": _thumbHeight + "px",
            "margin-right": (i < rowArray.length - 1 ? spacing + "px" : "0px"),
            "margin-bottom": spacing + "px"
        });

        _newCurrentWidth += _thumbWidth + (i < rowArray.length - 1 ? spacing : 0);
        rowArray[i].currentThumbWidth = _thumbWidth;
        rowArray[i].currentThumbHeight = _thumbHeight;
    }

    return newHeight + spacing; // row height + bottom spacing
}
